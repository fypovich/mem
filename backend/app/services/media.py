...
class MediaProcessor:
    ...
    def convert_to_gif(self, output_path: str):
        """
        Конвертирует видео в оптимизированный GIF с сохранением прозрачности.
        """
        try:
            # 1. Входной поток
            ...
            # 3. Генерация палитры из первого потока
            # 🔥 reserve_transparent=1: Важно для сохранения прозрачности
            palette = split[0].filter('palettegen', reserve_transparent=1)
            
            # 4. Применение палитры ко второму потоку
            # 🔥 alpha_threshold=128: Критично! Заставляет полупрозрачные пиксели 
            # (которые появляются при ресайзе) становиться полностью прозрачными, а не черными.
            out = ffmpeg.filter([split[1], palette], 'paletteuse', alpha_threshold=128)
            ...
            # Фолбэк: если сложный фильтр не сработал
            try:
                (
                    ffmpeg
                    .input(self.path)
