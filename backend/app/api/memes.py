...
# Импортируем все необходимые модели, включая SubjectCategory и таблицу follows
...
async def upload_meme(
    ...
    try:
        ...
        if audio_file:
            audio_path = os.path.join(UPLOAD_DIR, f"audio_{file_id}.{audio_file.filename.split('.')[-1]}")
            ...
            # Если это уже видео, ffmpeg может его оптимизировать, но пока просто переименуем если это mp4
            # Для надежности лучше прогнать через ffmpeg всегда, чтобы унифицировать формат
            processor.process_video_with_audio(None, final_path) # Метод должен уметь работать без аудио (если доработали) или просто rename
            # В текущей версии processor.process_video_with_audio требует аудио. 
            # Упростим: если аудио нет, просто переименуем (или конвертируем если это картинка)
            # ПРИМЕЧАНИЕ: В MediaProcessor нужно иметь метод convert_to_mp4_silent. 
            # Для надежности сейчас сделаем rename, предполагая что фронт шлет валидное.
            if os.path.exists(raw_path) and raw_path != final_path:
                 # Если логика process_video_with_audio сложная, лучше оставить как было в рабочем варианте
                 # Допустим, мы просто переименовываем для MVP
                 import shutil
...
async def read_memes(
    ...
    if liked_by:
        ...
        if target_user_id:
            ...
            return [] # Пользователь не найден, возвращаем пусто
    ...
    # --- СОРТИРОВКА ---
    if sort == "popular":
        # Сортируем по просмотрам и лайкам (комбинированный вес можно усложнить)
        # Здесь сортируем по лайкам, затем по просмотрам
    ...
    # Сборка ответа (Pydantic не умеет мапить кортежи (Meme, int, int, bool) напрямую в плоскую структуру, помогаем ему)
    memes_with_stats = []
    for row in rows:
        meme = row[0]
...
async def read_meme(
    ...
    # Используем update statement для атомарности
    await db.execute(
    ...
    query = (
        select(
            Meme,
            select(func.count(LikeStats.user_id)).where(LikeStats.meme_id == Meme.id).scalar_subquery(),
            select(func.count(CommentStats.id)).where(CommentStats.meme_id == Meme.id).scalar_subquery(),
            (exists().where((MyLike.meme_id == Meme.id) & (MyLike.user_id == current_user.id)) if current_user else sa.literal(False))
        )
        .options(selectinload(Meme.user), selectinload(Meme.tags), selectinload(Meme.subject))
...
async def like_meme(
    ...
    # Проверяем существование мема и автора
    ...
    if existing_like:
        ...
        if meme.user_id != current_user.id:
            # Проверяем, не было ли уже уведомления (чтобы не спамить при лайк/анлайк/лайк)
            # В реальном проде можно ставить cooldown или unique constraint
            notif = Notification(
...
async def create_comment(
    ...
    if meme.user_id != current_user.id:
        notif = Notification(
            ...
            meme_id=meme.id,
            text=new_comment.text[:50] # Обрезаем длинный текст для превью
        )
        db.add(notif)
...
async def check_meme_status(meme_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    """Проверка статуса обработки (для поллинга фронтенда)"""
    meme = await db.get(Meme, meme_id)
    if not meme: return {"status": "not_found"}
    return {"status": meme.status, "media_url": meme.media_url}